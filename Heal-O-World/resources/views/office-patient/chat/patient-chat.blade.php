...
<style>
  .chat-container {
    display: flex;
    height: calc(100vh - 100px); /* трошки нижче хедера */
...
<div class="chat-container">
    <aside class="chat-sidebar">
        <h3 class="text-xl font-bold mb-4 text-gray-800">Ваші чати</h3>

        @forelse($chats as $chat)
            @php
                $doctor = $chat->doctor;
                $initials = strtoupper(mb_substr($doctor->first_name, 0, 1) . mb_substr($doctor->last_name, 0, 1));
                $lastMessage = $chat->messages->last();
            @endphp
            <a href="{{ route('chat.show', $chat->id) }}" class="chat-card">
                <div class="avatar">{{ $initials }}</div>
                <div class="chat-info">
                    <div class="chat-name">{{ $doctor->fullName() ?? 'Лікар' }}</div>
