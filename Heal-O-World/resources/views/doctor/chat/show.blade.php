...
<div class="chat-container">
    <h2 class="chat-header">
        Чат з пацієнтом: {{ $chat->patient->first_name ?? '—' }} {{ $chat->patient->last_name ?? '' }}
    </h2>

    <div class="messages">
        @if($chat->messages->isEmpty())
            <p style="text-align:center; color:#95a5a6; font-style: italic;">Повідомлень ще немає. Почніть спілкування!</p>
        @else
    ...
    <form action="{{ route('doctor.chat.send', $chat->id) }}" method="POST" class="chat-form" autocomplete="off">
        @csrf
